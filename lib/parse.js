    ...
    const parser = new htmlparser.Parser(
      new htmlparser.DomHandler((err, dom) => {
        ...
        if (parsed) {
          ...
          const tag = match[1];
          const type =
            getTypeFromType(attributes.type) || getTypeFromTag(match[1]);
          const sourcepath =
            attributes.src || attributes.href || attributes.data;

          // Empty sourcepath attribute will be resolved as "true", so skip
          // Skip link tags with rel=preload/prefetch/prerender
          if (
            sourcepath === true ||
            (attributes.rel && RE_REL_PRE.test(attributes.rel))
          ) {
            return;
          }
