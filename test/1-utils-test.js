...
describe('utils', function() {
  ...
  describe('getAttributeString()', function() {
    ...
    it('should ignore blacklisted attributes', function() {
      expect(utils.getAttributeString({ foo: 'foo', inline: true, src: 'foo.js' }, 'inline', true)).to.eql(
        ' foo="foo"'
      );
    });
  });
  ...
  describe('getSourcepath()', function() {
    it('should return a resolved path for relative source path and default rootpath', function() {
      expect(utils.getSourcepath('./foo.css', path.resolve('./index.html'), process.cwd())).to.eql([
        path.resolve(process.cwd(), 'foo.css'),
        ''
      ]);
      expect(utils.getSourcepath('../css/foo.css', path.resolve('./html/index.html'), process.cwd())).to.eql([
    ...
    it('should return a resolved path for relative source path and custom rootpath', function() {
      expect(utils.getSourcepath('./foo.css', path.resolve('./index.html'), path.resolve('www'))).to.eql([
        path.resolve(process.cwd(), 'foo.css'),
        ''
      ]);
      expect(utils.getSourcepath('../css/foo.css', path.resolve('./html/index.html'), path.resolve('www'))).to.eql([
        path.resolve(process.cwd(), 'css/foo.css'),
        ''
      ]);
    });
    ...
    it('should return a resolved path for absolute source path and custom rootpath', function() {
      expect(
        utils.getSourcepath('/dist/images/foo.png', path.resolve('./dist/index.html'), path.resolve('./dist'))
      ).to.eql([path.resolve('dist/dist/images/foo.png'), '']);
      expect(utils.getSourcepath('/images/foo.png', path.resolve('./dist/index.html'), path.resolve('./dist'))).to.eql([
    ...
    it('should return anchor if present', function() {
      expect(utils.getSourcepath('./foo.svg#foo,bar', path.resolve('./index.html'), process.cwd())).to.eql([
        path.resolve(process.cwd(), 'foo.svg'),
        'foo,bar'
      ]);
    });
    it('should strip query params if present', function() {
      expect(utils.getSourcepath('./foo.css?v=12345', path.resolve('./index.html'), process.cwd())).to.eql([
        path.resolve(process.cwd(), 'foo.css'),
        ''
      ]);
      expect(utils.getSourcepath('./foo.css?', path.resolve('./index.html'), process.cwd())).to.eql([
      ...
      expect(utils.getSourcepath('./foo.svg?v=12345#foo,bar', path.resolve('./index.html'), process.cwd())).to.eql([
        path.resolve(process.cwd(), 'foo.svg'),
        'foo,bar'
      ]);
      expect(utils.getSourcepath('./foo.svg?#foo,bar', path.resolve('./index.html'), process.cwd())).to.eql([
